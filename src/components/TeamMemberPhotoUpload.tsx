...
export function TeamMemberPhotoUpload({
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Subir a S3
      const uploadResponse = await userPhotoService.uploadPhoto(file);

      if (uploadResponse.success) {
        const newPhotoUrl = uploadResponse.data.photoUrl;
        const newS3Key = uploadResponse.data.s3Key;

        // Actualizar el usuario en el backend (usando endpoint de consultorio para doctores)
        await userService.updateConsultorioUser(userId, {
  ...
  const handleRemovePhoto = async () => {
    ...
    try {
      ...
      // Eliminar de S3
      await userPhotoService.deletePhoto(s3Key);
